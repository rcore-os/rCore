...
pub struct usb_device_descriptor {
    pub bLength: u8,												// +0x0 Length of this descriptor
    pub bDescriptorType: u8,										// +0x1 Descriptor type
    pub bcdUSB: u16,												// +0x2 (in BCD 0x210 = USB2.10)
    pub bDeviceClass: u8,											// +0x4 Class code (enum DeviceClass )
    pub bDeviceSubClass: u8,										// +0x5 Subclass code (assigned by the USB-IF)
    pub bDeviceProtocol: u8,										// +0x6 Protocol code (assigned by the USB-IF)
    pub bMaxPacketSize0: u8,										// +0x7 Maximum packet size for endpoint 0
    pub idVendor: u16,												// +0x8 Vendor ID (assigned by the USB-IF)
    pub idProduct: u16,												// +0xa Product ID (assigned by the manufacturer)
    pub bcdDevice: u16,												// +0xc Device version number (BCD)
    pub iManufacturer: u8,											// +0xe Index of String Descriptor describing the manufacturer.
    pub iProduct: u8,												// +0xf Index of String Descriptor describing the product
    pub iSerialNumber: u8,											// +0x10 Index of String Descriptor with the device's serial number
    pub bNumConfigurations: u8,										// +0x11 Number of possible configurations
}
/*--------------------------------------------------------------------------}
...
pub struct UsbInterfaceDescriptor {
    ...
    pub Number: u8,													// +0x2 Number of this interface (0 based).
    pub AlternateSetting: u8,										// +0x3 Value of this alternate interface setting
    pub EndpointCount: u8,											// +0x4 Number of endpoints in this interface
    pub Class: InterfaceClass,										// +x05 Class code (assigned by the USB-IF)
    pub SubClass: u8,												// +x06 Subclass code (assigned by the USB-IF)
    pub Protocol: u8,												// +x07 Protocol code (assigned by the USB-IF)
    pub StringIndex: u8,											// +x08 Index of String Descriptor describing the interface
}
/*--------------------------------------------------------------------------}
...
pub struct UsbEndpointDescriptor {
    pub Header: UsbDescriptorHeader,								// +0x0 Length of this descriptor, +0x1 DEVICE descriptor type (enum DescriptorType)
    #[repr(align(1))]
    pub EndpointAddress: u8,										// +0x2  Endpoint address. Bit 7 indicates direction (0=OUT, 1=IN).
    #[repr(align(1))]
    pub Attributes: u8,												// +0x3 Endpoint transfer type
    #[repr(align(1))]
    pub Packet: u16,												// +0x4 Maximum packet size.
    pub Interval: u8,												// +0x6 Polling interval in frames
}
/*--------------------------------------------------------------------------}
...
pub struct HubDescriptor {
    ...
    pub DeviceRemovable: u8,										// +0x7
    pub PortPowerCtrlMask: u8,										// +0x8
}